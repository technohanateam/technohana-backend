...
import { generateToken, verfiyToken } from "../config/jwt.js";
...
router.get(
  ...
  (req, res) => {
    ...
    console.log('Environment variables:', {
      WHITELISTED_URLS: process.env.WHITELISHTED_URLS,
    });

    // Redirect back to the frontend with our token
    const frontendUrl = process.env.WHITELISHTED_URLS;
...
router.put("/kyc", async (req, res) => { // Changed from POST to PUT and simplified the path
  try {
    ...
    const payload = verfiyToken(token); // Assuming you have a verifyToken function
    if (!payload) return res.status(401).json({ message: "Invalid token" });
