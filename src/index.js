...
const allowedOrigins = process.env.WHITELISHTED_URLS
  ? process.env.WHITELISHTED_URLS.split(',').map(url => url.trim())
  : [];
...
console.log(process.env.WHITELISHTED_URLS);
...
app.post('/stripe/checkout', async (req, res) => {
  try {
    ...
    const frontendUrl = process.env.FRONTEND_URL || (process.env.WHITELISHTED_URLS ? process.env.WHITELISHTED_URLS.split(',')[0] : '');
    if (!frontendUrl) {
      return res.status(500).json({ error: 'Server misconfiguration: FRONTEND_URL/WHITELISHTED_URLS not set' });
    }
