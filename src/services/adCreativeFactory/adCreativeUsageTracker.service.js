...
// billing, good enough for the budget-tracking UI. Mirrors Content Factory's
// aiUsageTracker.service.js table (kept separate here since it rolls a
// different settings singleton's todaySpendUsd).
...
// Non-blocking accounting: writes an AiUsageLog row (callType prefixed
// "ad-creative-*" to stay distinguishable in the shared log) and rolls
// AdCreativeFactorySettings.todaySpendUsd forward, independent of
// ContentFactorySettings' own spend tracking.
export async function recordAdCreativeAiUsage({ model, tier = null, tokensIn = 0, tokensOut = 0, callType, opportunityId = null, jobId = null }) {
  try {
    const estimatedCostUsd = estimateCostUsd(model, tokensIn, tokensOut);

    await AiUsageLog.create({
      date: todayStr(),
      callType: `ad-creative-${callType || "unknown"}`,
      model: model || null,
      tier,
      tokensIn,
      tokensOut,
    ...
    console.error("[AdCreativeFactory] recordAdCreativeAiUsage failed (non-blocking):", err.message);
    return 0;
  }
}

// Drop-in wrapper for aiAgent.service.js's callClaude() — same signature plus
// callType/opportunityId/jobId, same { text, usage, model } return shape.
export async function trackedCallClaude({ system, prompt, maxTokens = 1024, tier = "standard", callType, opportunityId = null, jobId = null }) {
  ...
  await recordAdCreativeAiUsage({ model: result.model, tier, tokensIn, tokensOut, callType, opportunityId, jobId });
  return result;
}
